...
A map that flashes and changes color, similar to the nuclear
test map.
...
define(
  ...
  function (
    ...
    function panel2 (data, map) {
      ...
      // The reference to the animation function.
      var interval ;
      ...
      // Set up the blur filter for the glow effect.
      ...
      var text = "The problem is global but is especially acute in Latin America and Asia-Pacific." ;
      ...
      // a nice non-distorting projection from the d3.geo.projection plugin
      var projection = d3.geo.kavrayskiy7()
      .scale(160)
      .translate([c.width/2-50,c.height/2+15]) ;

      // path object for drawing on the map
      // (used to construct country shapes, arcs, etc.)
      var path = d3.geo.path()
      .projection(projection) ;

      // Group the data by country, then get the values
      // associated with each country. Use this to create a color scale.
      var grouped = _.groupBy(data, function (d) { return d["Geographical Term"]}) ;
      //var brazilless = _.omit(grouped, "Brazil");
      var cE = d3.extent(_.values(grouped).map(function (d) { return d.length; })) ;

      var fillScale = d3.scale.log()
      .domain([cE[0],(u.sum(cE)/2), cE[1]])
      .clamp(true)
      .range(["#FFF685", "#e61719", "#9A0000"]) ;

      // Create a key for the color scale.

      var colorKey = vizBox.append("g")
      .attr("transform", "translate(35,200)") ;

      var colorKeyValues = u.expscale(1.133, 50);// [2,4,8,16,32,64,128,256,512];

      var keyBoxHeight = 4,
          keyBoxWidth = 16,
          keyHeight = keyBoxHeight * colorKeyValues.length ;

      colorKey.selectAll("colorkey")
      .data(colorKeyValues)
      .enter()
      .append("rect")
      .attr("class", "colorkey")
      .attr("width",keyBoxWidth)
      .attr("height",keyBoxHeight)
      .attr("y",function (d,i) {return i*keyBoxHeight;})
      .attr("x", 0)
      .attr("fill", function (d) {return fillScale(d);}) ;

      var increments = [10,50,125,250,500] ;

      var colorKeyScale = d3.scale.log()
      .domain(d3.extent(colorKeyValues))
      .range([0,keyHeight]) ;

      colorKey.selectAll("colorkey-label")
      .data(increments)
      .enter()
      .append("foreignObject")
      .attr("width", "2em")
      ...
      function drawMap () {
        ...
        var theMap = vizBox.selectAll("path.glow")
        ...
        // The OTHER text box...
        // The time ticker for the map.
        var timeTicker = vizBox.append("text")
        .attr("transform", "translate(" + 735 + "," +
              (c.vizHeight) + ")")
        .attr("class", "mapdate") ;
      ...
      function totalCountAt (name, time) {
        /*
        Returns the total number of cases associated with a given country
        at a given time.
        */
        if (!grouped[name]) {
          return 0 ;
        } ;
        var cases = grouped[name] ;
        var casesLtd = cases.filter(function (d) {
          return (d[c.datefield].getTime() <= time) ;
        }) ;
        return casesLtd.length ;
      } ;

      function flashDatum (d, time) {
        function pred (e) {
          return (e.id === d["Geographical Term"]) ;
        } ;
        var countries = vizBox.selectAll(".land").filter(pred);
        var glows = vizBox.selectAll(".glow").filter(pred);
        var overs = vizBox.selectAll(".over").filter(pred);

        var glowAttack = 50,
            glowDecay = 350 ;

        // immediate changes
        countries.style("fill", function (e) { return fillScale(totalCountAt(e.id, time)) ;}) ;
        overs.attr("opacity", 1)
        glows.style("fill", function (e) { return fillScale(totalCountAt(e.id, time)) ;}) ;

        // transition the layover's fill to the new color
        var overT0 = overs.transition().duration(10) ;
        overT0.style("fill", function (e) { return fillScale(totalCountAt(e.id, time)) ;}) ;

        // eventually remove the layover
        var overT1 = overT0.transition().delay(glowDecay - glowAttack) ;

        // make the glow appear...
        var glowT0 = glows.transition().duration(glowAttack) ;
        glowT0.style("opacity", 1) ;

        // ...and disappear
        var glowT1 = glowT0.transition().duration(glowDecay) ;
        glowT1.style("opacity", 0) ;
      ...
      function tickThrough () {
        ...
        */
        ...
        // reset the visibility of the ticker.
        d3.select(".mapdate").style("opacity", 1) ;

        interval = setInterval(
          function () {
            if (currentPoint <= max) {
              var currentSlice = data.filter(function (d) {
                var t = d[c.datefield].getTime() ;
                return ((t <= currentPoint) && (t > prevPoint)) ;
              }) ;
              currentSlice.forEach(function (d) {flashDatum(d,currentPoint)}) ;
              d3.select(".mapdate").text(d3.time.format("%Y")(new Date(currentPoint)));
      ...
      function showTips () {
        // This function, when called, causes captions to display
        // alongside the list of "worst offenders".
        // This is all quite ad hoc, since manual adjustments
        // have to be made to the locations of the captions anyway.
        var worstOffenders = [
          "Brazil",
          "Colombia",
          "Honduras",
        ...
        function loc (d) {
          // A silly kludge to adjust the location of the caption boxes.
          var c = path.centroid(d);
          switch (d.id) {
            case "Brazil":
              return [c[0]+50,c[1]-16];
            case "Colombia":
              return [c[0]+20,c[1]-30];
              break;
            case "Honduras":
              return [c[0]-130,c[1]-25];
              break;
            case "Philippines":
              return [c[0]+15,c[1]-25];
            case "Peru":
              return [c[0]-145,c[1]-25];
            default:
              return c;
          } ;
        } ;

        var worstCountries = vizBox.selectAll(".over")
        .filter(function (e) {return _.contains(worstOffenders,e.id); });

        var max = data[data.length-1][c.datefield].getTime();
        worstCountries.each(function (e) {
          var container = vizBox.append("foreignObject")
          .style("opacity",0)
          .attr("height", "8em")
          .attr("width", "12em")
          .attr("transform",
          ...
          .html("<strong>" + e.id + "</strong><br>" +
                "<strong>Killings</strong>: " + totalCountAt(e.id, max)) ;
        }) ;
      ...
      return {
        init: function () { drawMap();
                           tickThrough(); },
        kill: function () { vizBox.selectAll(".land").remove();
                           vizBox.selectAll(".glow").remove();
                           vizBox.selectAll(".over").remove();
                           vizBox.selectAll(".d3-tip").remove();
                           clearInterval(interval);}
      } ;
